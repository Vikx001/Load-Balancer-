...
def make_app(profile: dict) -> aiohttp.web.Application:
    ...
    async def handle(request: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        return aiohttp.web.Response(status=200, text=body, content_type="application/json")
    ...
    async def handle_admin(request: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        try:
            ...
            s = _state[bid]
            if "base_ms" in data:
                s["base_ms"] = float(data["base_ms"])
            if "jitter" in data:
                s["jitter"] = float(data["jitter"])
            if "error_pct" in data:
                s["error_pct"] = float(data["error_pct"])
            if "overload" in data:
                s["overload"] = bool(data["overload"])
...
async def run_backend(profile: dict):
    ...
    await site.start()
    print(
        f"[backends] Backend {profile['id']} -> http://{BACKEND_HOST}:{profile['port']}  "
