...
class TestKANInferenceSymbolic(unittest.TestCase):
    ...
    def test_output_sums_to_one(self):
        w = self._infer([0.4, 0.6, 0.2, 0.5], [45, 55, 120, 40], [0.001, 0.002, 0.01, 0.001], [True] * 4)
    ...
    def test_output_shape(self):
        w = self._infer([0.1, 0.2, 0.3], [30, 40, 50], [0.0, 0.0, 0.0], [True] * 3)
        self.assertEqual(w.shape, (3,))

    # ── Health masking ───────────────────────────────────────────────────

    def test_unhealthy_backend_gets_zero_weight(self):
        w = self._infer([0.1, 0.1, 0.1, 0.1], [30, 30, 30, 30], [0.0] * 4, [True, False, True, True])
    ...
    def test_low_cpu_gets_more_weight(self):
        ...
        self.assertGreater(float(w[0]), float(w[1]), "low-CPU backend should get more weight")

    def test_high_error_rate_reduces_weight(self):
        w = self._infer([0.3, 0.3], [50, 50], [0.001, 0.1], [True, True])
        self.assertGreater(float(w[0]), float(w[1]), "high error rate backend should get less weight")

    def test_high_latency_reduces_weight(self):
        w = self._infer([0.3, 0.3], [50, 500], [0.001, 0.001], [True, True])
        self.assertGreater(float(w[0]), float(w[1]), "high latency backend should get less weight")

    # ── Equations ────────────────────────────────────────────────────────

    def test_equations_count(self):
        eqs = self.kan.equations(np.array([0.3, 0.5]), np.array([40.0, 60.0]), np.array([0.001, 0.002]), [True, True])
        self.assertEqual(len(eqs), 2)

    def test_equations_format(self):
        eqs = self.kan.equations(np.array([0.3]), np.array([40.0]), np.array([0.001]), [True])
    ...
    def test_stats_recorded(self):
        for _ in range(5):
            self._infer([0.3] * 3, [40.0] * 3, [0.001] * 3, [True] * 3)
...
class TestCBFProjector(unittest.TestCase):
    ...
    def test_output_sums_to_one(self):
        w, _ = self.cbf.project(np.array([0.4, 0.3, 0.2, 0.1]), np.array([0.5, 0.9, 0.3, 0.6]))
        self.assertAlmostEqual(float(w.sum()), 1.0, places=5)

    def test_output_all_nonnegative(self):
        w, _ = self.cbf.project(np.array([0.25] * 4), np.array([0.7] * 4))
    ...
    def test_overloaded_backend_weight_reduced(self):
        ...
        self.assertLess(float(w[0]), float(raw_norm[0]), "CBF should reduce weight on overloaded backend")

    def test_cbf_fires_on_overloaded_backend(self):
        _, fired = self.cbf.project(np.array([0.9, 0.05, 0.03, 0.02]), np.array([0.95, 0.2, 0.2, 0.2]))
    ...
    def test_safe_loads_leave_weights_unchanged(self):
        ...
        w, fired = self.cbf.project(raw, loads)
        raw_norm = raw / raw.sum()
        np.testing.assert_allclose(w, raw_norm, atol=0.05, err_msg="Safe loads: weights should not change much")
        self.assertFalse(any(fired), "No CBF should fire when loads are safe")

    def test_cbf_does_not_fire_when_safe(self):
        _, fired = self.cbf.project(np.array([0.25] * 4), np.array([0.2, 0.3, 0.1, 0.4]))
    ...
    def test_per_backend_caps_respected(self):
        ...
        _, fired = self.cbf.project(np.array([0.4, 0.2, 0.2, 0.2]), loads, caps=caps)
    ...
    def test_detailed_result_fields(self):
        ...
        result = self.cbf.project_detailed(np.array([0.7, 0.1, 0.1, 0.1]), np.array([0.9, 0.3, 0.3, 0.3]))
...
class TestSafetyMonitor(unittest.TestCase):
    ...
    def test_step_returns_valid_simplex(self):
        w, fired = self.monitor.step(np.array([0.4, 0.3, 0.2, 0.1]), np.array([0.5, 0.3, 0.2, 0.4]))
        self.assertAlmostEqual(float(w.sum()), 1.0, places=5)

    def test_violation_rate_increases_under_overload(self):
        for _ in range(10):
            self.monitor.step(np.array([0.8, 0.1, 0.05, 0.05]), np.array([0.9, 0.2, 0.2, 0.2]))
    ...
    def test_violation_rate_zero_when_safe(self):
        for _ in range(10):
            self.monitor.step(np.array([0.25] * 4), np.array([0.1, 0.2, 0.15, 0.1]))
        for i in range(4):
            self.assertAlmostEqual(self.monitor.violation_rate(i), 0.0, places=6)

    def test_audit_dict_structure(self):
        self.monitor.step(np.array([0.25] * 4), np.array([0.1] * 4))
        audit = self.monitor.audit()
        required_keys = {"total_projections", "total_violations", "mean_iters_per_step", "violation_rates", "cap"}
        self.assertTrue(required_keys.issubset(audit.keys()), f"Missing keys: {required_keys - audit.keys()}")
    ...
    def test_recent_violations_returns_list(self):
        ...
        for _ in range(5):
            self.monitor.step(np.array([0.9, 0.04, 0.03, 0.03]), np.array([0.95, 0.2, 0.2, 0.2]))
...
class TestLBSimEnv(unittest.TestCase):
    ...
    def test_reset_gives_different_states(self):
        ...
        self.assertFalse(np.allclose(s1, s2), "Different seeds should give different initial states")

    def test_episode_terminates(self):
        self.env.reset()
        done = False
        for _ in range(3000):
            _, _, terminated, truncated, _ = self.env.step(np.array([0.25] * 4))
    ...
    def test_equal_weights_distribute_load(self):
        ...
        self.assertLess(max_imbalance, 0.5, f"Equal weights produced imbalanced loads: {cpu}")
    ...
    def test_concentrated_weights_increase_overloaded_backend(self):
        ...
        cpu = self.env.loads / self.env.capacities
        self.assertGreater(float(cpu[0]), float(cpu[1]), "Backend-0 should be more loaded than others")
...
class TestKANCBFPipeline(unittest.TestCase):
    ...
    def test_pipeline_output_is_valid_simplex(self):
        w, _ = self._pipeline_step(
            [0.4, 0.6, 0.2, 0.5], [45, 55, 120, 40], [0.001, 0.002, 0.01, 0.001], [True] * 4, [0.5, 0.9, 0.3, 0.6]
        )
        self.assertAlmostEqual(float(w.sum()), 1.0, places=5)
        self.assertTrue((w >= 0).all())
    ...
    def test_pipeline_fires_on_overload(self):
        ...
        _, fired = self._pipeline_step(
            [0.3, 0.95, 0.3, 0.3], [40, 200, 40, 40], [0.001, 0.05, 0.001, 0.001], [True] * 4, [0.2, 0.95, 0.2, 0.2]
        )
    ...
    def test_pipeline_handles_single_healthy_backend(self):
        ...
        w, _ = self._pipeline_step([0.3] * 4, [50] * 4, [0.001] * 4, [False, True, False, False], [0.1] * 4)
        self.assertAlmostEqual(float(w.sum()), 1.0, places=5)
