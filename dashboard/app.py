...
SIM_BL = np.array([45.0, 52.0, 120.0, 38.0])
SIM_BU = np.array([0.30, 0.35, 0.45, 0.28])
SIM_PATHS = [
    ("/api/v1/health", "GET"),
    ("/api/v1/users", "GET"),
    ("/api/v2/ingest", "POST"),
...
def _init_sim():
    if "tick" in st.session_state:
        return
    s = st.session_state
    s.tick = 0
    s.rng = np.random.default_rng(42)
    s.lat = np.zeros((N_SIM, H_SIM))
    s.load = np.zeros((N_SIM, H_SIM))
    s.rps = np.zeros(H_SIM)
    s.err = np.zeros((N_SIM, H_SIM))
    s.vnodes = np.array([150.0] * N_SIM)
    s.health = [True] * N_SIM
    s.rl = np.array([1000.0] * N_SIM)
    s.cbf = [False] * N_SIM
    s.proactive = False
    s.wt = np.ones(N_SIM) / N_SIM
    s.fail = -1
    s.spike = False
    s.treq = [0] * N_SIM
    s.logs = []
...
def _gen_sim_logs(rng, w, lat, err, rps):
    s = st.session_state
    n_new = max(3, min(10, int(rps / 300)))
    now = datetime.now()
    new = []
    sw = w / w.sum() if w.sum() > 0 else np.ones(N_SIM) / N_SIM
    for _ in range(n_new):
        i = int(rng.choice(N_SIM, p=sw))
        path, method = SIM_PATHS[int(rng.integers(len(SIM_PATHS)))]
        lv = max(1.0, lat[i] + rng.normal(0, lat[i] * 0.15))
        er = err[i]
        if not s.health[i]:
            level, status, ls, tag = "ERROR", 503, "—", "CONN_REFUSED"
        elif rng.random() < er:
            level = "ERROR"
            status = int(rng.choice([500, 502, 503]))
            ls = f"{lv:.0f}ms"
            tag = "UPSTREAM_ERR"
        elif lv > 300:
            level, status, ls, tag = "WARN", 200, f"{lv:.0f}ms", "SLOW_RESPONSE"
        else:
            level, status, ls, tag = "INFO", 200, f"{lv:.0f}ms", ""
        ms = int(rng.integers(0, 999))
        ts = now.strftime("%H:%M:%S") + f".{ms:03d}"
...
def _gen_live_logs(N, lat, err, rps, health, names):
    ...
    n_new = max(2, min(8, int(rps / 400)))
    now = datetime.now()
    new = []
    ww = np.array([1.0 if health[i] else 0.0 for i in range(N)])
    if ww.sum() == 0:
        ww = np.ones(N)
    ww /= ww.sum()
    for _ in range(n_new):
        i = int(rng.choice(N, p=ww))
        path, method = SIM_PATHS[int(rng.integers(len(SIM_PATHS)))]
        lv = max(1.0, lat[i] + rng.normal(0, lat[i] * 0.15))
        er = err[i]
        if not health[i]:
...
def _delta(cur, prev, invert=False):
    diff = cur - prev
    pct = diff / abs(prev) * 100 if abs(prev) > 0.001 else 0
    if abs(pct) < 0.5:
        return '<span class="fl">Stable vs 30s ago</span>'
    a = "▲" if diff > 0 else "▼"
    cls = ("up" if diff > 0 else "dn") if not invert else ("dn" if diff > 0 else "up")
...
n_up = sum(health)
cur_rps = float(rps[-1])
prev_rps = float(rps[-6]) if H > 6 else cur_rps
avg_lat = float(np.mean([lat[i, -1] for i in range(N) if health[i]])) if n_up else 0.0
avg_latp = float(np.mean([lat[i, -6] for i in range(N) if health[i]])) if n_up and H > 6 else avg_lat
avg_err = float(np.mean([err[i, -1] * 100 for i in range(N) if health[i]])) if n_up else 0.0
...
if n_up == N and cbf_cnt == 0:
    ...
    pill_c, pill_l = "p-warn", "Degraded"
else:
    pill_c, pill_l = "p-err", "Critical"
...
with T[0]:
    if mode == "demo":
        st.markdown(
            f'<div class="mode-banner-demo"><b>DEMO mode</b> — simulation data. Start the proxy to see real traffic. See the <b>Setup</b> tab.</div>',
            unsafe_allow_html=True,
        )
    else:
        age = round(time.time() - d["proxy_ts"], 1)
        st.markdown(
            f'<div class="mode-banner-live">● <b>LIVE</b> — connected to proxy · data age {age}s · tick #{d["tick"]}</div>',
            unsafe_allow_html=True,
        )

    k = st.columns(4, gap="small")
    kpis = [
        ("Requests / sec", f"{cur_rps:,.0f}", _delta(cur_rps, prev_rps), C["blue"], rps[-30:]),
        (
            "Avg Latency (P50)",
            f"{avg_lat:.0f} ms",
            _delta(avg_lat, avg_latp, True),
            C["purple"],
            lat[:, -30:].mean(axis=0),
        ),
    ...
    for i in range(N):
        ...
        fig_g.add_trace(
            go.Indicator(
                ...
                title={
                    "text": f"<b style='font-family:JetBrains Mono,monospace;font-size:11px;color:{C['muted']};'>{NAMES[i][:12]}</b><br><span style='font-size:10px;color:{C['dim']};'>{latv:.0f}ms · {ev:.2f}% err</span>",
                    "font": {"family": "Inter"},
                },
                gauge={
                    "axis": {"range": [0, 100], "tickwidth": 0, "visible": False},
                    "bar": {"color": col_g, "thickness": 0.22},
                    "bgcolor": "rgba(0,0,0,0)",
                    "borderwidth": 0,
                    "steps": [
                        {"range": [0, 70], "color": C["surface2"]},
                        {"range": [70, 80], "color": C["amber_d"]},
                        {"range": [80, 100], "color": C["red_d"]},
                    ],
                    "threshold": {"line": {"color": C["red"], "width": 2}, "thickness": 0.8, "value": 80},
                },
            ),
            row=1,
            col=i + 1,
    ...
    with ca:
        fig_rps = go.Figure(
            go.Scatter(
                ...
                fill="tozeroy",
                fillcolor="rgba(61,142,240,0.06)",
                hovertemplate="%{y:,.0f} req/s<extra></extra>",
            )
        )
        fig_rps.update_layout(
            **{
                **PL,
                "title": dict(text="Requests per Second", font=dict(size=13, color=C["text"]), x=0, xanchor="left"),
                "height": 240,
                "yaxis": dict(**PL["yaxis"], title=dict(text="req/s", font=dict(size=10))),
                "xaxis": dict(**PL["xaxis"], title=dict(text="seconds", font=dict(size=10))),
            }
        )
        st.markdown('<div class="card">', unsafe_allow_html=True)
    ...
    with c2:
        ...
        for i in range(N):
            fig2.add_trace(
                go.Scatter(
                    ...
                    y=load[i] * 100,
                    name=NAMES[i],
                    line=dict(color=COLS[i], width=1.5),
                    opacity=0.3 if not health[i] else 1.0,
                    hovertemplate=f"{NAMES[i]}: %{{y:.1f}}%<extra></extra>",
                )
            )
        fig2.add_hline(
            y=80,
            line=dict(color=C["red"], width=1, dash="dot"),
            annotation_text="CBF cap (80%)",
        ...
        fig2.update_layout(
            **{
                **PL,
                "title": dict(text="Target Utilization (%)", font=dict(size=13, color=C["text"]), x=0, xanchor="left"),
                "height": 230,
                "yaxis": dict(**PL["yaxis"], ticksuffix="%", range=[0, 105]),
            }
        )
        st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            if health[i]:
                fig3.add_trace(
                    go.Scatter(
                        ...
                        name=NAMES[i],
                        line=dict(color=COLS[i], width=1.5),
                        hovertemplate=f"{NAMES[i]}: %{{y:.0f}} ms<extra></extra>",
                    )
                )
        fig3.update_layout(
            **{
                **PL,
                "title": dict(text="Latency — P50 (ms)", font=dict(size=13, color=C["text"]), x=0, xanchor="left"),
                "height": 230,
                "yaxis": dict(**PL["yaxis"], ticksuffix=" ms"),
            }
        )
        st.markdown('<div class="card">', unsafe_allow_html=True)
    ...
    for i in range(N):
        ...
        ev = err[i, -1] * 100
        if not health[i]:
            sb = f'<span class="pill p-err"><span class="dot"></span>Unhealthy</span>'
        elif ev > 2 or latv > 400:
            sb = f'<span class="pill p-warn"><span class="dot"></span>Degraded</span>'
        else:
            sb = f'<span class="pill p-ok"><span class="dot"></span>Healthy</span>'
        cb2 = (
            f'<span class="cbf-fire">FIRING</span>'
            if cbf[i]
        ...
        with st.expander("Fault Simulation", expanded=False):
            ...
            with fc:
                ...
                for i in range(N_SIM):
                    s.health[i] = i != s.fail
            with sc2:
                s.spike = st.toggle("Traffic spike (2.4×)", value=False, key="spike_tog")
            with rc:
                cr = st.columns(N_SIM)
                for i, cx in enumerate(cr):
                    s.rl[i] = cx.slider(f"B{i}", 200, 2000, int(s.rl[i]), 100, key=f"rl_{i}")
...
with T[1]:
    ...
    with ra:
        ...
        for ti in range(H):
            lv2 = lat[:, ti]
            inv2 = np.where(lv2 > 0, 1.0 / (lv2 + 1), 0.0)
        ...
        fig_w.update_layout(
            **{
                **PL,
                "title": dict(
                    text="KAN Routing Weight Distribution (%)", font=dict(size=13, color=C["text"]), x=0, xanchor="left"
                ),
                "height": 250,
                "yaxis": dict(**PL["yaxis"], ticksuffix="%", range=[0, 105]),
            }
        ...
        st.markdown(
            '<div style="height:16px"></div><div class="sh">CBF Safety Projection</div>', unsafe_allow_html=True
        )
        eq_rows = ""
        for i in range(N):
            cpu_c = load[i, -1]
    ...
    with rb:
        ...
        fig_d = go.Figure(
            go.Pie(
                values=vnodes / vnodes.sum() * 100,
                labels=NAMES,
                hole=0.62,
                marker=dict(colors=COLS, line=dict(color=C["surface"], width=2)),
                textinfo="none",
                sort=False,
                direction="clockwise",
        ...
        fig_d.add_annotation(
            ...
            y=0.5,
            showarrow=False,
            font=dict(color=C["text"], family="Inter", size=20),
            align="center",
        )
        fig_d.update_layout(
            height=200,
        ...
        for i in range(N):
            ...
            st.markdown(
                f'<div style="margin-bottom:14px;"><div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span class="mono" style="font-size:11px;color:{C["muted"]}">{NAMES[i][:12]}</span><span style="font-size:11px;color:{C["text"]};font-variant-numeric:tabular-nums">{pct2:.1f}%</span></div><div style="height:4px;background:{C["border"]};border-radius:2px;"><div style="width:{pct2}%;height:4px;background:{COLS[i]};border-radius:2px;"></div></div><div style="font-size:10px;color:{C["dim"]};margin-top:2px;">{int(vnodes[i])} vnodes</div></div>',
                unsafe_allow_html=True,
            )
        wh3 = vnodes / tot * np.array([1.0 if h else 0.0 for h in health])
        nh = max(1, n_up)
        if wh3.sum() > 0:
            wh3 /= wh3.sum()
        bf = float(wh3.max()) / (1.0 / nh)
        bfc = C["green"] if bf <= 1.1 else (C["amber"] if bf <= 1.3 else C["red"])
        st.markdown(
            f'<div style="background:{C["surface2"]};border:1px solid {C["borderl"]};border-radius:8px;padding:14px 16px;"><div style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:{C["muted"]};margin-bottom:4px;">Balance Factor</div><div style="font-size:24px;font-weight:700;color:{bfc};letter-spacing:-.03em;">{bf:.3f}</div><div style="font-size:10px;color:{C["dim"]};margin-top:2px;">ideal 1.000 · cap ≤ 1.100</div></div>',
            unsafe_allow_html=True,
        )
        if proactive:
            st.markdown(
                f'<div style="background:{C["amber_d"]};border:1px solid {C["amber"]};border-radius:6px;padding:10px 12px;margin-top:10px;font-size:11px;color:{C["amber"]};"><b>Proactive redistribution active</b><br><span style="color:{C["muted"]}">Rising load slope detected.</span></div>',
                unsafe_allow_html=True,
            )
...
with T[2]:
    ...
    for i, col in enumerate(rc_c):
        arr = (vnodes[i] / vnodes.sum()) * cur_rps
        util = min(1.0, arr / rl[i]) if health[i] else 0
        lv3 = load[i, -1]
        atxt, ac = (
            ("Throttling ↓", C["red"])
            if lv3 > 0.80
    ...
    fig_rl = go.Figure()
    for i in range(N):
        ah = (vnodes[i] / vnodes.sum()) * rps
        uth = np.minimum(1.0, ah / rl[i]) * 100
        fig_rl.add_trace(
            go.Scatter(
                x=ts_ax,
                y=uth,
    ...
    fig_rl.add_hline(
        y=90,
        line=dict(color=C["red"], width=1, dash="dot"),
        annotation_text="Rate limit threshold",
        annotation_font_size=10,
        annotation_font_color=C["red"],
    )
    fig_rl.update_layout(
        **{
            **PL,
            "title": dict(
                text="Token Bucket Utilization (%)", font=dict(size=13, color=C["text"]), x=0, xanchor="left"
            ),
            "height": 240,
            "yaxis": dict(**PL["yaxis"], ticksuffix="%", range=[0, 105]),
        }
    ...
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.plotly_chart(fig_rl, width="stretch", config={"displayModeBar": False})
    st.markdown("</div>", unsafe_allow_html=True)
...
with T[3]:
    ...
    with ha:
        ...
        for i in range(N):
            ...
            uv4 = load[i, -1] * 100
            if not health[i]:
                bdr, sl4, sc4 = C["red"], "Unhealthy", "p-err"
            elif ev4 > 2 or lv4 > 400:
                bdr, sl4, sc4 = C["amber"], "Degraded", "p-warn"
            else:
    ...
    with hb:
        ...
        fig_p.update_layout(
            **{
                **PL,
                ...
                "bargroupgap": 0.06,
                "height": 230,
                "title": dict(text="Latency Percentiles", font=dict(size=13, color=C["text"]), x=0, xanchor="left"),
                "yaxis": dict(**PL["yaxis"], ticksuffix=" ms"),
            }
        )
        st.markdown('<div class="card">', unsafe_allow_html=True)
        ...
        st.markdown("</div>", unsafe_allow_html=True)
        st.markdown(
            '<div style="height:14px"></div><div class="sh">Latency Heatmap — last 60 ticks</div>',
            unsafe_allow_html=True,
        )
        fig_hm = go.Figure(
            go.Heatmap(
                z=lat[:, -60:],
                x=np.arange(-59, 1),
                y=[n[:10] for n in NAMES],
                colorscale=[[0, C["green_d"]], [0.25, C["green"]], [0.6, C["amber"]], [1, C["red"]]],
                colorbar=dict(
                    title=dict(text="ms", side="right"),
                    thickness=10,
                    len=0.9,
                    tickfont=dict(size=9, color=C["muted"]),
                    outlinewidth=0,
                ),
                hovertemplate="<b>%{y}</b><br>t=%{x}s<br>%{z:.0f} ms<extra></extra>",
                zmin=0,
                zmax=500,
                xgap=1,
                ygap=2,
        ...
        fig_hm.update_layout(
            ...
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",
            margin=dict(t=10, b=28, l=80, r=60),
            xaxis=dict(showgrid=False, zeroline=False, tickfont=dict(size=9, color=C["dim"])),
            yaxis=dict(showgrid=False, zeroline=False, tickfont=dict(size=10, color=C["muted"])),
        )
        st.markdown('<div class="card">', unsafe_allow_html=True)
...
with T[4]:
    ...
    steps = [
        ...
        (
            ...
            "One-time setup — creates a virtual environment:",
            "python3 -m venv .venv\n.venv/bin/pip install -r requirements.txt",
        ),
        (
            "Edit omega-lb.yaml",
            "Replace the example backends with your real service addresses. Supports 2–8 backends:",
            'backends:\n  - host: "192.168.1.10"\n    port: 8001\n    name: "api-prod-1"\n    zone: "dc-a"\n  - host: "192.168.1.11"\n    port: 8001\n    name: "api-prod-2"\n    zone: "dc-b"',
        ),
        (
            "Start the proxy",
            "Listens on port 8080 by default (change in omega-lb.yaml). Point your load generator at it:",
            "python demo/proxy.py\n# or one-command:  ./start.sh",
        ),
        (
            "Open this dashboard",
            "Run in a separate terminal — it auto-detects the proxy:",
            "# already running if you used ./start.sh\n.venv/bin/streamlit run dashboard/app.py",
        ),
    ]
    for n_step, (title, desc, code) in enumerate(steps, 1):
        st.markdown(
            f'<div class="setup-step"><div class="step-num">{n_step}</div><div class="step-body"><div class="step-title">{title}</div><div class="step-desc">{desc}</div><div class="code-block">{code}</div></div></div>',
            unsafe_allow_html=True,
        )

    # Config editor
    st.markdown(
        '<div style="height:20px"></div><div class="sh">omega-lb.yaml — edit and save</div>', unsafe_allow_html=True
    )
